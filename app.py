...
# ============================================================
# CUSTOM PATCHEXTRACT LAYER FOR ViT
# ============================================================
...
# ============================================================
# LOAD DEEP MODELS (CNN + CNN+ViT) WITH ERROR HANDLING
# ============================================================
@st.cache_resource
def load_dl_models():
    import os
    ...
    cnn_vit_path = "cnn_vit_model.h5"

    if not os.path.exists(cnn_path):
        st.error(f"File not found: {cnn_path}")
        return None, None
    if not os.path.exists(cnn_vit_path):
        st.error(f"File not found: {cnn_vit_path}")
        return None, None

    cnn_model = load_model(cnn_path)
    cnn_vit_model = load_model(cnn_vit_path, custom_objects={"PatchExtract": PatchExtract})
...
if uploaded_file is not None:
    ...
    # ANN
    ann = MLPClassifier(hidden_layer_sizes=(64,32), activation="relu", solver="adam", max_iter=1500, random_state=42)
    ...
    if cnn_model is not None and cnn_vit_model is not None:
        ...
        cnn_result = vit_result = "Model not loaded"
